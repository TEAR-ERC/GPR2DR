...
RECURSIVE SUBROUTINE InitParameters(STRLEN,PARSETUP) 
    ...
    EQN%Yeq_mode= 6                     ! Mode 4 is the more general one allowing all the rupture processes  through parameters Ya,Yb,Yc,s0
...
RECURSIVE SUBROUTINE InitialData(xGP, tGp, Q)
	...
	REAL :: ICA,ICsig,ICxd,cp,k1,k2,xi0
	...
	case('NLOPRUPTURECG')
	   ...
	   !rr=-(xGP(2)-2000)
        ...
        up(20)    = LamCoeff(2)
        up(19)    = LamCoeff(3)   
       if(abs(xGP(1))<70000.0 .and. abs(xGP(2))<50.0) then
        up(22) = 1.0
       else
        up(22)=0.0
       end if
        ...
        EQN%p0    = 0.0
        EQN%tau0  = 1.e-6 ! 1e-6
        EQN%Yeq_mode =6 !1=Yeq; 6= westerly
        ! -----------------------------------------------------------------------------------------------
        ! Compute the initial stress --------------------------------------------------------------------------------
         LEsigma=0.

       !if(abs(xGP(1)) .le. 15000.0 .and. abs(xGP(2)) .le. 200.0 )then
        !LEsigma(1)=-120.0*1.e+6
        LEsigma(2)=-120.0*1.e+6
	...
	!end if
        ...
        if(nDim .eq. 3) then
            ...
            x1 = xGP(1)
            y1 = xGP(2)
               if(abs(x1)<70000.0 .and. abs(y1)<50.0) then
                   up(21)= 1.0
               end if
           ! k1 = 200.0
           ! k2 = 3*k1
           ! CALL random_number(xi0)
           ! up(21)  = min(1.0, exp(-10.0*(y1 - 0)**2/k1**2) + xi0*exp(-10.0*(y1 - 0)**2/k2**2))
